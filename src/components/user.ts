...
export default {
  template: `<div class="refresher-user" :data-me="me" v-on:click="clickHandle" :class="{cursor: !!this.user.id}" v-on:contextmenu="contextMenu" :title="(user.id ? '(' + user.id + ')' : user.ip ? '(' + user.ip + (user.ip_data ? ', ' + (user.ip_data.length > 100 ? user.ip_data.substring(0, 97) + '...' : user.ip_data) : '') + ')' : '').replaceAll('\\n', ' ').replaceAll(/  +/g, ' ')">
